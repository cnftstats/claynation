...
X[price %between% c(1000,  4999),  price_range := trunc(price/500)*500]
X[price %between% c(5000,  9999),  price_range := trunc(price/1000)*1000]
X[price %between% c(10000, 14999), price_range := trunc(price/5000)*5000]
...
X[price_range %between% c(1000,  4999),  x_label := paste0(price_range, "-", price_range + (500-1))]
X[price_range %between% c(5000,  9999),  x_label := paste0(price_range, "-", price_range + (1000-1))]
X[price_range %between% c(10000, 14999), x_label := paste0(price_range, "-", price_range + (5000-1))]
...
```{r}
max_y <- max(DT[, quantile(price, probs = .90)], DTS[, max(price)])
min_y <- min(DT[, min(price)], DTS[, min(price)])
X <- DTS[price <= max_y]
X[, sold_time := trunc(sold_at_days)]
X[, sold_time := fcase(trunc(sold_at_days) == 0, "0-24h ago",
                       trunc(sold_at_days) == 1, "24-48h ago",
...
gg <- ggplot(X, aes(asset_rank, price, color = sold_time, alpha = sold_time,
  ...
  scale_alpha_manual(values = c("0-24h ago" = 1,
                                "24-48h ago" = 0.6,
                                "48-72h ago" = 0.2)) +
  labs(x = "Rank", y = "Sold price", color = "Sold time", alpha = "Sold time") +
  theme(legend.title = element_text(size = 10))
